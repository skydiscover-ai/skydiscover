...
9. **All sizing must be runtime-parameterized.** Hash table buckets, cache entries, buffer pool pages, arena sizes -- ALL must be computed from `mem_budget_bytes` inside `InitExtended()`, not hardcoded as compile-time constants. Your code must work correctly at ANY `mem_budget_bytes` value (1 GB, 8 GB, 32 GB, 256 GB). A canary test at a different budget will catch violations.
10. **Values are variable-sized per key.** Each `GenValue` carries its own `.size` field. Store and return exactly `value.size` bytes -- never truncate or assume a fixed size. The workload's typical value size (from the trace card) is an optimization hint for layout and alignment, not a hard constraint. For fixed-slot pre-allocation use `GenValue::kMaxSize` (4096); for better memory efficiency use a variable-length arena.

### Required: `ReadAsync` + `CompletePending`

Both are pure-virtual. If your reads never block (pure in-memory), copy:
...
If reads can block (disk, network, contended eviction), return
`OpStatus::Pending` without waiting and complete the slot from
...
- Keys are monotone timestamps. Inserts append at tail, deletes only
  remove from head.
- After the run, the harness samples ~1024 keys from `[0, head)` and
  ~1024 from `[head, tail)` and calls `Read()` on each. Deleted keys
